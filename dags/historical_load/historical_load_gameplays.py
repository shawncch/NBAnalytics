...
def retrieve_games_for_week(game_date) -> list[str]:
    # print(game_date)
    # convert to python date
    date = datetime.strptime(game_date, '%Y-%m-%d').date()
    game_dates = [date + timedelta(days=i) for i in range(7)]

    games_for_week_df = []
    for game_date in game_dates:
        games_for_week_df.append(retrieve_games_for_day(game_date))
    
    # print(games_for_week_df)
    return games_for_week_df
...
def retrieve_plays_for_week(ti):
    game_list = ti.xcom_pull(task_ids='get_week_games')

    for games_per_day in game_list:
        for index, row in games_per_day.iterrows():
            game_id = row['GAME_ID']
            game_date = row['GAME_DATE']
            season_id = row['SEASON_ID']
            nba_season = get_nba_season(game_date)
            if season_id[0] not in ['2', '4', '6']:
                continue
